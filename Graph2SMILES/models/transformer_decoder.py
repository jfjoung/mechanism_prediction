"""
Copied / adapted from onmt.decoder.transformer
"""
...
class TransformerDecoder(DecoderBase):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
                 ...
                 alignment_heads, latent_embedding):
    ...
    def forward(self, tgt, memory_bank, step=None, **kwargs):
        ...
        # log_tensor(emb, "emb", shape_only=True)
        if self.latent_embedding is not None:
            latent_emb = self.latent_embedding(kwargs["latent_classes"])        # [b,] -> [b, h]
            if kwargs["mode"] == "train":
                latent_emb = torch.unsqueeze(latent_emb, dim=0)                 # [b, h] -> [1, b, h] to be broadcast
            elif kwargs["mode"] == "predict":
                latent_emb = torch.reshape(latent_emb, [1, 1, -1])              # [h,] -> [1, 1, h] to be broadcast
            else:
                raise NotImplementedError
            # log_tensor(latent_emb, "latent_emb")
            # exit(0)
            # latent_emb = torch.unsqueeze(latent_emb, dim=0)                     # [b, h] -> [1, b, h] to be broadcast
            # latent_emb = torch.reshape(latent_emb, [1, 1, -1])                  # [h,] -> [1, 1, h] to be broadcast
            # latent_emb = latent_emb.repeat(1, emb.size()[1] // latent_emb.size()[1], 1)
            # log_tensor(latent_emb, "latent_emb", shape_only=True)
            emb = emb + latent_emb
