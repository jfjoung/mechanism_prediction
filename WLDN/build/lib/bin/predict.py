...
# def beam_search(args, smiles, predictor, beam_width=3, max_steps=7):
#     smol = Chem.MolFromSmiles(smiles, sanitize=False)
#     for atom in smol.GetAtoms():
#         atom.SetAtomMapNum(0)
#     smiles = Chem.MolToSmiles(smol, isomericSmiles=False)
#     rxn_dict = {1: {'reaction': [smiles], 'prob': [1], 'predicted_edits': [[]], 'score': [0]}}
#
#
#
#     for i in range(max_steps):
#         new_rxn_dict = {}
#         for key, val in rxn_dict.items():
#             if i != 0 and val['predicted_edits'][-1] != []:
#                 res = predictor.predict_single(val['reaction'][-1])
#                 for r in res:
#                     if r['smiles'] in val['reaction'][:-1]:
#                         removing_smi=r['smiles']
#                         res=adjust_probability(res, removing_smi)
#             elif i == 0:
#                 res = predictor.predict_single(val['reaction'][-1])
#                 res = adjust_probability(res, smiles)
#             else:
#                 res = [{'rank': 1, 'smiles': None, 'predicted_edits': [], 'score': 0, 'prob': 1}]
#             for r in res:
#                 new_reaction = val['reaction'] + [r['smiles']]
#                 new_prob = val['prob'] + [r['prob']]
#                 new_predicted_edits = val['predicted_edits'] + [r['predicted_edits']]
...
#                                                  'score': new_score}
#
#         sorted_items = sorted(new_rxn_dict.items(), reverse=True)
#         rxn_dict = dict(sorted_items[:beam_width])
#     return rxn_dict
...
if __name__ == '__main__':
    ...
    # res = predictor.predict_single(args.smiles)
    # print('res is ',res)
    #
